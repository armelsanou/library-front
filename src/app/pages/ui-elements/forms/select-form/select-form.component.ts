...
export class SelectFormComponent implements OnInit, OnDestroy {
  ...
  simpleOption: Array<IOption> = this.selectOptionService.getCharacters(this.PLAYER_ONE);
  selectedOption = '3';
  isDisabled = true;
  ...
  ngOnInit() {
    this.runTimer();
    this.dataSub = this.selectOptionService.loadCharacters(this.PLAYER_ONE).subscribe((options) => {
